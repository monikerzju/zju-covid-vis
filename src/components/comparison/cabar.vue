...
export default {
  ...
  methods: {
    cabarInit () {
      ...
      const getDate = () => this.dataOri[0][this.dateIndex]
      let date = getDate()
      let dataSlice = []
      let chart = null
      let scale = null
      ...
      // 格式化数据
      const formatData = () => {
        this.dataOri[0].forEach((date, index) => {
          if (index > 0) {
            this.dataOri.forEach((row, rowIndex) => {
              if (rowIndex > 0) {
                data.push({
                  name: row[0],
      ...
      const renderAxis = () => {
        ...
        axis = (d3.axisTop().scale(scale).ticks(5).tickPadding(3).tickSize(0))

        svg.append('g')
          .classed('axis', true)
          .style('transform', `translate3d(${margin.left + 100}px, ${margin.top}px, 0)`)
          .call(axis)
      }
    ...
    async switchDate (opt) {
      let dateSet = this.dataOri[0]
      let index = dateSet.indexOf(opt)
      if (index === -1) {
        this.dateIndex = 0
      } else {
